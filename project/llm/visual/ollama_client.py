...
class OllamaClient(LLMClientBase):
    ...
    def _query_text_only(self, prompt: str) -> str:
        try:
            ...
            if raw_usage["prompt_tokens"] is not None and raw_usage["completion_tokens"] is not None:
                raw_usage["total_tokens"] = int(raw_usage["prompt_tokens"]) + int(raw_usage["completion_tokens"])
                self._set_last_usage(self._normalize_provider_usage(raw_usage))
            else:
